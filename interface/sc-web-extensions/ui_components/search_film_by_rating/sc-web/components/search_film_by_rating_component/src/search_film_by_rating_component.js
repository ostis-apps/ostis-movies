SearchFilmByRating = {
    ext_lang: 'search_film_by_rating_code',
    formats: ['format_search_film_by_rating_json'],
    struct_support: true,

    factory: function (sandbox) {
        return new setSearchFilmByRatingViewerWindow(sandbox);
    }
};

var setSearchFilmByRatingViewerWindow = function (sandbox) {
    ...
    var keynodes = ['ui_search_film_by_rating_in_memory'];

    $('#' + sandbox.container).prepend('<div class="inputBox" id="series-tools-' + sandbox.container + '"></div>');
    $('#series-tools-' + sandbox.container).load('static/components/html/search_film_by_rating_component.html', function () {
        SCWeb.core.Server.resolveScAddr(keynodes, function (keynodes) {
            SCWeb.core.Server.resolveIdentifiers(keynodes, function (idf) {
                var buttonSearch = idf[keynodes['ui_search_film_by_rating_in_memory']];
                ...
                $(buttonFind).click(function () {
                    ...
                    console.log("Film find " + fromString);
                    console.log("Film find cinocompany string" + rtoString);
    ...
    this.applyTranslation = function (namesMap) {
        SCWeb.core.Server.resolveScAddr(keynodes, function (keynodes) {
            SCWeb.core.Server.resolveIdentifiers(keynodes, function (idf) {
                var buttonLoad = idf[keynodes['ui_search_film_by_rating_in_memory']];
...
SCWeb.core.ComponentManager.appendComponentInitialize(SearchFilmByRating);

function findFilmByRating(searchParams) {
    ...
    SCWeb.core.Server.resolveScAddr([searchParams.fromRating, searchParams.toRating], function (keynodes) {
        ...
        SCWeb.core.Server.resolveScAddr(["ui_menu_file_for_finding_film_by_rating"], function (data) {
            // Get command of ui_menu_view_full_semantic_neighborhood
            var cmd = data["ui_menu_file_for_finding_film_by_rating"];
            console.log("cmd", cmd);
